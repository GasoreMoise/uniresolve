...
import { Search, FileText, CheckCircle, CheckCircle2, Clock, AlertTriangle, RefreshCw, Eye, Loader2, LogOut, X, Filter, FolderOpen, ArrowLeft, BookOpen } from 'lucide-react';
...
import GradeEntryModal from '@/components/staff/GradeEntryModal';
...
type ViewMode = 'QUEUE' | 'GRADEBOOK';

export default function LecturerDashboardLedger() {
  ...
  const [activeModal, setActiveModal] = useState<'NONE' | 'APPROVE' | 'REJECT' | 'ACTION_REQUIRED' | 'EXAM_RESOLVE' | 'TRANSCRIPT_RESOLVE' | 'GRADE_ENTRY'>('NONE');

  // Gradebook States
  const [roster, setRoster] = useState<any[]>([]);
  const [activeGradeStudent, setActiveGradeStudent] = useState<any | null>(null);
  const [activeGradeModule, setActiveGradeModule] = useState<{id: string, code: string} | null>(null);
  ...
  const fetchClassRoster = async () => {
    try {
      const data = await api.grades.getRoster();
      setRoster(data);
    } catch (err) {
      console.error("Failed to fetch academic roster");
    }
  };
  ...
  return (
    <div className="min-h-screen w-full bg-slate-100 text-slate-900 font-sans flex flex-col justify-between overflow-x-hidden select-none">
      ...
      <main className="grow max-w-6xl w-full mx-auto px-6 py-10 space-y-6">
        ...
        <div className="flex items-center gap-4 bg-white p-2 rounded border border-slate-200 shadow-sm w-max">
          <button 
            ...
            className={`px-6 py-2 rounded text-xs font-bold uppercase tracking-wider transition cursor-pointer ${dashboardMode === 'QUEUE' ? 'bg-[#2B35AF] text-white shadow' : 'text-slate-500 hover:bg-slate-50 border-none bg-transparent'}`}
          >
            Claims Queue
          </button>
          <button 
            onClick={() => setDashboardMode('GRADEBOOK')}
            className={`px-6 py-2 rounded text-xs font-bold uppercase tracking-wider transition cursor-pointer ${dashboardMode === 'GRADEBOOK' ? 'bg-emerald-600 text-white shadow' : 'text-slate-500 hover:bg-slate-50 border-none bg-transparent'}`}
          >
            Academic Gradebook
          </button>
        </div>

        {dashboardMode === 'GRADEBOOK' ? (
          /* ========================================================== */
          /* GRADEBOOK WORKSPACE VIEW                                   */
          /* ========================================================== */
          <div className="space-y-8 animate-in fade-in duration-300">
            {roster.length === 0 ? (
              <div className="bg-white border border-slate-200 rounded p-12 text-center shadow-sm">
                <BookOpen size={48} className="text-slate-300 mx-auto mb-4" />
                <h2 className="text-lg font-bold text-slate-800 mb-2">No Modules Assigned</h2>
                <p className="text-sm text-slate-500">You are currently not mapped to any active academic modules.</p>
              </div>
            ) : (
              roster.map((module) => (
                <div key={module.id} className="bg-white border border-slate-200 rounded overflow-hidden shadow-sm">
                  <div className="bg-slate-50 p-4 border-b border-slate-200 flex justify-between items-center">
                    <div>
                      <h2 className="text-sm font-bold text-[#2B35AF]">{module.code}</h2>
                      <p className="text-xs text-slate-500 font-medium mt-0.5">{module.title}</p>
                    </div>
                    <div className="bg-white border border-slate-200 text-slate-500 text-[10px] font-bold px-3 py-1.5 rounded uppercase tracking-widest">
                    ...
                    <table className="w-full text-left text-xs">
                      <thead>
                        <tr className="bg-white border-b border-slate-100 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                          ...
                          <th className="px-6 py-3 text-center">Attempt</th>
                          <th className="px-6 py-3 text-center">Standing Score</th>
                          <th className="px-6 py-3 text-right">Ledger Action</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-50">
                        {module.students.map((student: any) => (
                          <tr key={student.userId} className="hover:bg-slate-50/50 transition">
                            <td className="px-6 py-3 font-mono font-medium text-slate-500">{student.registrationNumber}</td>
                            <td className="px-6 py-3 font-bold text-slate-800">{student.fullName}</td>
                            <td className="px-6 py-3 text-center">
                              <span className={`px-2 py-0.5 text-[9px] font-bold rounded border ${student.enrollmentType === 'FIRST_ATTEMPT' ? 'bg-blue-50 text-blue-600 border-blue-200' : 'bg-amber-50 text-amber-600 border-amber-200'}`}>
                                {student.enrollmentType.replace('_', ' ')}
                              </span>
                            </td>
                            <td className="px-6 py-3 text-center">
                              {student.grade && student.grade.finalScore !== null ? (
                                ...
                                <span className="text-slate-400 italic text-[10px]">Awaiting</span>
                              )}
                            </td>
                            <td className="px-6 py-3 text-right">
                              <button 
                                onClick={() => {
                                  setActiveGradeModule({ id: module.id, code: module.code });
                                  setActiveGradeStudent(student);
                                  setActiveModal('GRADE_ENTRY');
                                }}
                                className="inline-flex items-center gap-1.5 bg-slate-900 hover:bg-slate-800 text-white font-bold py-1.5 px-3 rounded text-[10px] uppercase tracking-wide transition cursor-pointer border-none"
                              >
                                <BookOpen size={11} /> Update Ledger
                              </button>
                            </td>
                          </tr>
                        ))}
                        {module.students.length === 0 && (
                          <tr>
                            <td colSpan={5} className="px-6 py-8 text-center text-slate-400 italic">No students registered in this module.</td>
                          </tr>
                        )}
                      </tbody>
                    </table>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                  ...
                  <div className="space-y-4 animate-in fade-in duration-300">
                    ...
                    {Object.keys(groupedQueue).length === 0 ? (
                      <div className="bg-white border border-slate-200 rounded p-12 text-center text-slate-400 text-xs italic shadow-sm">
                        {isLoading ? 'Syncing ledger categories...' : 'No service requests found matching your current filters.'}
                      </div>
                    ) : (
              ...
              {inspectingTicket && (
                <div className="lg:col-span-5 bg-white border border-slate-200 rounded p-5 space-y-4 shadow-sm animate-in slide-in-from-right-4 duration-200 relative text-xs sticky top-6">
                  ...
                  {/* DISPLAY TARGETED MODULE IF APPLICABLE */}
                  {inspectingTicket.module && (
                    <div className="bg-slate-50 border border-slate-200 rounded p-3 mb-2 flex items-center justify-between">
                      <div>
                        <span className="block text-[9px] font-bold text-slate-400 uppercase tracking-wider">Targeted Module</span>
                        <span className="font-mono font-bold text-[#2B35AF]">{inspectingTicket.module.code}</span>
                      </div>
                      <div className="text-right">
                        <span className="block text-[9px] font-bold text-slate-400 uppercase tracking-wider">Lecturer</span>
                        <span className="font-bold text-slate-700">{inspectingTicket.module.lecturer?.fullName || 'Unassigned'}</span>
                      </div>
                    </div>
                  )}
      ...
      {/* NEW: GRADE ENTRY MODAL FOR ACADEMIC GRADEBOOK */}
      {activeModal === 'GRADE_ENTRY' && activeGradeStudent && activeGradeModule && (
        <GradeEntryModal 
          isOpen={true}
          onClose={() => setActiveModal('NONE')}
          studentId={activeGradeStudent.userId}
          studentName={activeGradeStudent.fullName}
          moduleId={activeGradeModule.id}
          moduleCode={activeGradeModule.code}
          existingGrade={activeGradeStudent.grade}
          onSaved={() => {
            setActiveModal('NONE');
            fetchClassRoster();
          }}
